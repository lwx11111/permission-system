...
@RestController
@Tag(name = "服务")
@RequestMapping("/sysnoticereceiver")
public class SysNoticeReceiverController {
    @Autowired
    private ISysNoticeReceiverService service;

    @PostMapping
    @ResponseBody
    @Operation(description = "创建")
    public SimpleResponse save(@RequestBody SysNoticeReceiver obj){
    ...
    public SimpleResponse update(@PathVariable(name = "id") String id,@RequestBody SysNoticeReceiver obj){
    ...
    public SimpleResponse selectBy(@RequestBody(required = false) Map<String, String> params) {
        List<SysNoticeReceiver> sysnoticereceivers = service.selectBy(params);
        return new SimpleResponse.SimpleResponseBuilder().success(sysnoticereceivers).build();
    }
    ...
    public SimpleResponse selectPage(@RequestBody Map<String, String> params) {
        try {
            IPage<SysNoticeReceiver> page = service.selectPage(params);
    ...
    public SimpleResponse selpageCustomSqlByWrapper(@RequestBody Map<String, String> params) {
        try {
            IPage<SysNoticeReceiver> page = service.selpageCustomSqlByWrapper(params);
            return new SimpleResponse.SimpleResponseBuilder().success(page).build();
        } catch (Exception e) {
            e.printStackTrace();
            return new SimpleResponse.SimpleResponseBuilder().failure(e.getMessage()).build();
    ...
    public SimpleResponse selpageCustomSqlByMap(@RequestBody Map<String, String> params) {
        try {
            IPage<SysNoticeReceiver> page = service.selpageCustomSqlByMap(params);
            return new SimpleResponse.SimpleResponseBuilder().success(page).build();
        } catch (Exception e) {
            e.printStackTrace();
            return new SimpleResponse.SimpleResponseBuilder().failure(e.getMessage()).build();
