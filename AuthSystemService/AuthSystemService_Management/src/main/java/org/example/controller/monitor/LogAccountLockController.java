...
@RestController
@Tag(name = "服务")
@RequestMapping("/logaccountlock")
public class LogAccountLockController {
    @Autowired
    private ILogAccountLockService service;

    @PostMapping
    @ResponseBody
    @Operation(description = "创建")
    public SimpleResponse save(@RequestBody LogAccountLock obj){
    ...
    public SimpleResponse update(@PathVariable(name = "id") String id,@RequestBody LogAccountLock obj){
    ...
    public SimpleResponse selectBy(@RequestBody(required = false) Map<String, String> params) {
        List<LogAccountLock> logaccountlocks = service.selectBy(params);
        return new SimpleResponse.SimpleResponseBuilder().success(logaccountlocks).build();
    }
    ...
    public SimpleResponse selectPage(@RequestBody Map<String, String> params) {
        try {
            IPage<LogAccountLock> page = service.selectPage(params);
    ...
    public SimpleResponse selpageCustomSqlByWrapper(@RequestBody Map<String, String> params) {
        try {
            IPage<LogAccountLock> page = service.selpageCustomSqlByWrapper(params);
            return new SimpleResponse.SimpleResponseBuilder().success(page).build();
        } catch (Exception e) {
            e.printStackTrace();
            return new SimpleResponse.SimpleResponseBuilder().failure(e.getMessage()).build();
    ...
    public SimpleResponse selpageCustomSqlByMap(@RequestBody Map<String, String> params) {
        try {
            IPage<LogAccountLock> page = service.selpageCustomSqlByMap(params);
            return new SimpleResponse.SimpleResponseBuilder().success(page).build();
        } catch (Exception e) {
            e.printStackTrace();
            return new SimpleResponse.SimpleResponseBuilder().failure(e.getMessage()).build();
