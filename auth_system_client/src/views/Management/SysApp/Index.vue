...
<script lang="ts" setup>
    import Api from '@/api/Management/api_sysapp.js'
    ...
    const getData = () => {
        ...
        // 查询参数
        const params = {
            appCode : data.formList.appCode,
            appName : data.formList.appName,
            icon : data.formList.icon,
            ...
            fixed : data.formList.fixed,
            secretKey : data.formList.secretKey,
            appKey : data.formList.appKey,
            status : data.formList.status,
            pageIndex : data.pageConfig.currentPage,
            pageSize : data.pageConfig.pageSize
        }
        // 后台请求
        Api.selpage4sysapp(params).then(res=> {
            if (res.code === 200){
    ...
    const deleteDataMany = () => {
        ...
        if (!datas || !datas.length || datas.length === 0) {
            ...
            ElMessageBox.confirm(
                ...
                Api.dels4sysapp(dataids).then(res => {
                    if (res.code === 200){
                        ElMessage({
                            type: 'success',
                            message: '删除成功',
    ...
    const excelData = () => {
        const params = {}
        Api.excelData4sysapp(params).then(data => {
            const blob = new Blob([data.data], { type: 'application/vnd.ms-excel' })
            const blobUrl = URL.createObjectURL(blob)
            const a = document.createElement('a')
            a.href = blobUrl
    ...
    const toUpdate = (scope) => {
        data.type = "update"
        itemDialog.value.init(scope.row.appId, data.type);
    }
    ...
    const toDelete = (scope) => {
        ElMessageBox.confirm(
            ...
            Api.del4sysapp(scope.row.appId).then(res => {
                if (res.code === 200){
                    ElMessage({
                        type: 'success',
                        message: '删除成功',
