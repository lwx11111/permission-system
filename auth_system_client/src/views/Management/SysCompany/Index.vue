                        ...
                        <el-form :inline="true"
                            ...
                            <el-form-item label="公司名称">
                                  <el-input placeholder="请输入内容"
                                            v-model="data.formList.companyName"
                                            style="width: 200px"
...
<script lang="ts" setup>
    import Api from '@/api/Management/api_syscompany.js'
    ...
    const getData = () => {
        ...
        // 查询参数
        const params = {
            companyName : data.formList.companyName,
            pageIndex : data.pageConfig.currentPage,
            pageSize : data.pageConfig.pageSize
        }
        // 后台请求
        Api.selpage4syscompany(params).then(res=> {
            if (res.code === 200){
    ...
    const deleteDataMany = () => {
        ...
        if (!datas || !datas.length || datas.length === 0) {
            ...
            ElMessageBox.confirm(
                ...
                Api.dels4syscompany(dataids).then(res => {
                    if (res.code === 200){
                        ElMessage({
                            type: 'success',
                            message: '删除成功',
    ...
    const excelData = () => {
        const params = {}
        Api.excelData4syscompany(params).then(data => {
            const blob = new Blob([data.data], { type: 'application/vnd.ms-excel' })
            const blobUrl = URL.createObjectURL(blob)
            const a = document.createElement('a')
            a.href = blobUrl
    ...
    const toUpdate = (scope) => {
        data.type = "update"
        itemDialog.value.init(scope.row.companyId, data.type);
    }
    ...
    const toDelete = (scope) => {
        ElMessageBox.confirm(
            ...
            Api.del4syscompany(scope.row.companyId).then(res => {
                console.log(res)
                if (res.code === 200){
                    ElMessage({
                        type: 'success',
