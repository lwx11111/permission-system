            ...
            <el-table
                ...
                <el-table-column
                    ...
                    <template #default="scope">
                        <NormalStatusColumn :status="scope.row.status"/>
                    </template>
                </el-table-column>
                <el-table-column
                        prop="funType"
                        ...
                        align="center">
                    <template #default="scope">
                        <el-tag size="medium">{{ scope.row.status==='1' ? '菜单' : '按钮' }}</el-tag>
                    </template>
                </el-table-column>
                 <el-table-column
                        prop="permKey"
...
<script lang="ts" setup>
    ...
    import NormalStatusColumn from "@/views/components/NormalStatusColumn.vue";
    ...
    const deleteDataMany = () => {
        ...
        if (!datas || !datas.length || datas.length === 0) {
            ...
            ElMessageBox.confirm(
                ...
                Api.dels4sysfunction(dataids).then(res => {
                    if (res.code === 200){
                        ElMessage({
                            type: 'success',
                            message: '删除成功',
    ...
    const excelData = () => {
        const params = {}
        Api.excelData4sysfunction(params).then(data => {
            const blob = new Blob([data.data], { type: 'application/vnd.ms-excel' })
            const blobUrl = URL.createObjectURL(blob)
            const a = document.createElement('a')
            a.href = blobUrl
    ...
    const toUpdate = (scope) => {
        data.type = "update"
        itemDialog.value.init(scope.row.funId, data.type);
    }
    ...
    const toDelete = (scope) => {
        ElMessageBox.confirm(
            ...
            Api.del4sysfunction(scope.row.funId).then(res => {
                console.log(res)
                if (res.code === 200){
                    ElMessage({
                        type: 'success',
