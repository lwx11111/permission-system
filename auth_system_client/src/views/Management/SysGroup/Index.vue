                        ...
                        <el-form :inline="true"
                            ...
                            <el-form-item label="组编码">
                                  <el-input placeholder="请输入内容"
                                            v-model="data.formList.groupCode"
                                            style="width: 200px"
                            ...
                            <el-form-item label="组名称">
                                  <el-input placeholder="请输入内容"
                                            v-model="data.formList.groupName"
                                            style="width: 200px"
                                            @keyup.enter.native="getData">
                                  </el-input>
                            </el-form-item>
...
<script lang="ts" setup>
    import Api from '@/api/Management/api_sysgroup.js'
    ...
    const getData = () => {
        ...
        // 查询参数
        const params = {
            groupCode : data.formList.groupCode,
            groupName : data.formList.groupName,
            pageIndex : data.pageConfig.currentPage,
            pageSize : data.pageConfig.pageSize
        }
        // 后台请求
        Api.selpage4sysgroup(params).then(res=> {
            if (res.code === 200){
    ...
    const deleteDataMany = () => {
        ...
        if (!datas || !datas.length || datas.length === 0) {
            ...
            ElMessageBox.confirm(
                ...
                Api.dels4sysgroup(dataids).then(res => {
                    if (res.code === 200){
                        ElMessage({
                            type: 'success',
                            message: '删除成功',
    ...
    const excelData = () => {
        const params = {}
        Api.excelData4sysgroup(params).then(data => {
            const blob = new Blob([data.data], { type: 'application/vnd.ms-excel' })
            const blobUrl = URL.createObjectURL(blob)
            const a = document.createElement('a')
            a.href = blobUrl
    ...
    const toUpdate = (scope) => {
        data.type = "update"
        itemDialog.value.init(scope.row.groupId, data.type);
    }
    ...
    const toDelete = (scope) => {
        ElMessageBox.confirm(
            ...
            Api.del4sysgroup(scope.row.groupId).then(res => {
                console.log(res)
                if (res.code === 200){
                    ElMessage({
                        type: 'success',
