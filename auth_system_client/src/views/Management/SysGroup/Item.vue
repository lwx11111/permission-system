    ...
    <el-dialog v-model="data.showDialog"
        ...
        <el-card style="border: 1px solid gold;"
            ...
            <el-tabs v-model="data.activeName" class="demo-tabs" @tab-click="handleClick">
                <el-tab-pane label="组属性" name="attribute">
                    <el-form
                        :model="data.item"
                        :rules="data.rules"
                        ref="itemForm"
                ...
                <el-tab-pane label="成员分配" name="assignment">
                    <el-transfer
...
<script lang="ts" setup>
    ...
    const init = (id, type) => {
        ...
        if (data.type === 'detail' || data.type === 'update') {
            ...
            // 发送请求
            Api.sel4sysgroup(data.id).then(res => {
                if (res.code === 200){
                    data.item = res.data;
                    // 界面显示
        ...
        Api.listMembers(params).then(res => {
            console.log(res)
            if (res.code === 200){
                data.allUsers  = res.data.allUsers.records
    ...
    const saveOrUpdate = () => {
        // 保存或更新操作
        if (data.type === 'update') {
            Api.update4sysgroup(data.id, data.item).then(res => {
                if (res.code === 200){
                    ElMessage({
                      message: '修改成功',
                      type: 'success',
            ...
            const params = {
                group: data.item,
                accountIds: data.members,
                functions : selFun,
                dataset : dataset
            }
            Api.add4sysgroup(params).then(res => {
                if (res.code === 200){
                    ElMessage({
                        message: '保存成功',
                        type: 'success',
