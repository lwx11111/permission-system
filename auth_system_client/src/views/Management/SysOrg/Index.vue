...
<!--                        <span><span class="icon iconfont" v-html="data.icon"></span> {{ node.label }}</span>-->
<!--                      </span>-->
<!--                        node-key="companyId"-->
                    </el-tree>
                </div>
                ...
                <el-table
                    ...
                    <el-table-column
                        ...
                        <template #default="scope">
                            <el-tag size="medium">{{ scope.row.status ==='1' ? '公司' :
                                scope.row.status ==='2' ? '部门' : '外部组织'}}</el-tag>
                        </template>
                    </el-table-column>
                    <el-table-column
                            prop="status"
                            ...
                            align="center">
                        <template #default="scope">
                            <el-tag size="medium">{{ scope.row.status==='1' ? '停用' : '正常' }}</el-tag>
                        </template>
                    </el-table-column>
                    <el-table-column
                            prop="orgManager"
...
<script lang="ts" setup>
    ...
    // Methods
    const handleNodeClick = (companyData, node, tree) => {
        data.formList.companyId= companyData.companyId
        getData()
    }
    ...
    const deleteDataMany = () => {
        ...
        if (!datas || !datas.length || datas.length === 0) {
            ...
            ElMessageBox.confirm(
                ...
                Api.dels4sysorg(dataids).then(res => {
                    if (res.code === 200){
                        ElMessage({
                            type: 'success',
                            message: '删除成功',
    ...
    const excelData = () => {
        const params = {}
        Api.excelData4sysorg(params).then(data => {
            const blob = new Blob([data.data], { type: 'application/vnd.ms-excel' })
            const blobUrl = URL.createObjectURL(blob)
            const a = document.createElement('a')
            a.href = blobUrl
    ...
    const toUpdate = (scope) => {
        data.type = "update"
        itemDialog.value.init(scope.row.orgCode, data.type);
    }
    ...
    const toDelete = (scope) => {
        ElMessageBox.confirm(
            ...
            Api.del4sysorg(scope.row.orgCode).then(res => {
                console.log(res)
                if (res.code === 200){
                    ElMessage({
                        type: 'success',
