                        ...
                        <el-form :inline="true"
                            ...
                            <el-form-item label="角色编码">
                                  <el-input placeholder="请输入内容"
                                            v-model="data.formList.roleCode"
                                            style="width: 200px"
                            ...
                            <el-form-item label="角色名字">
                                  <el-input placeholder="请输入内容"
                                            v-model="data.formList.roleName"
                                            style="width: 200px"
                                            @keyup.enter.native="getData">
                                  </el-input>
                            </el-form-item>
...
<script lang="ts" setup>
    ...
    const getData = () => {
        ...
        // 查询参数
        const params = {
            roleCode : data.formList.roleCode,
            appId : data.formList.appId,
            roleName : data.formList.roleName,
            orderCode : data.formList.orderCode,
            ...
            customRoleId : data.formList.customRoleId,
            pageIndex : data.pageConfig.currentPage,
            pageSize : data.pageConfig.pageSize
        }
        // 后台请求
        Api.selpage4sysrole(params).then(res=> {
            if (res.code === 200){
    ...
    const deleteDataMany = () => {
        ...
        if (!datas || !datas.length || datas.length === 0) {
            ...
            ElMessageBox.confirm(
                ...
                Api.dels4sysrole(dataids).then(res => {
                    if (res.code === 200){
                        ElMessage({
                            type: 'success',
                            message: '删除成功',
    ...
    const excelData = () => {
        const params = {}
        Api.excelData4sysrole(params).then(data => {
            const blob = new Blob([data.data], { type: 'application/vnd.ms-excel' })
            const blobUrl = URL.createObjectURL(blob)
            const a = document.createElement('a')
            a.href = blobUrl
    ...
    const toUpdate = (scope) => {
        data.type = "update"
        itemDialog.value.init(scope.row.roleId, data.type);
    }
    ...
    const toDelete = (scope) => {
        ElMessageBox.confirm(
            ...
            Api.del4sysrole(scope.row.roleId).then(res => {
                console.log(res)
                if (res.code === 200){
                    ElMessage({
                        type: 'success',
