                        ...
                        <el-form :inline="true"
                            ...
                            <el-form-item label="设置名称">
                                  <el-input placeholder="请输入内容"
                                            v-model="data.formList.settingName"
                                            style="width: 200px"
...
<script lang="ts" setup>
    import Api from '@/api/Management/api_syssetting.js'
    ...
    const getData = () => {
        ...
        // 查询参数
        const params = {
            settingCode : data.formList.settingCode,
            settingName : data.formList.settingName,
            settingVal : data.formList.settingVal,
        ...
        // 后台请求
        Api.selpage4syssetting(params).then(res=> {
            if (res.code === 200){
    ...
    const deleteDataMany = () => {
        ...
        if (!datas || !datas.length || datas.length === 0) {
            ...
            ElMessageBox.confirm(
                ...
                Api.dels4syssetting(dataids).then(res => {
                    if (res.code === 200){
                        ElMessage({
                            type: 'success',
                            message: '删除成功',
    ...
    const excelData = () => {
        const params = {}
        Api.excelData4syssetting(params).then(data => {
            const blob = new Blob([data.data], { type: 'application/vnd.ms-excel' })
            const blobUrl = URL.createObjectURL(blob)
            const a = document.createElement('a')
            a.href = blobUrl
    ...
    const toDelete = (scope) => {
        ElMessageBox.confirm(
            ...
            Api.del4syssetting(scope.row.id).then(res => {
                console.log(res)
                if (res.code === 200){
                    ElMessage({
                        type: 'success',
