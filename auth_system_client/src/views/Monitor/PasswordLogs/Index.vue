...
<script lang="ts" setup>
    import Api from '@/api/Monitor/api_logaccountchangepassword.js'
    ...
    const getData = () => {
        ...
        // 后台请求
        Api.selpage4logaccountchangepassword(params).then(res=> {
            if (res.code === 200){
    ...
    const downloadExcelTemplate = () => {
        ...
        Api.downloadExcelTemplate(params).then(data => {
            ...
            a.href = blobUrl
            a.download = 'LogAccountChangePassword.xls'
            a.click()
    ...
    const deleteDataMany = () => {
        ...
        if (!datas || !datas.length || datas.length === 0) {
            ...
            ElMessageBox.confirm(
                ...
                Api.dels4logaccountchangepassword(dataids).then(res => {
                    if (res.code === 200){
                        ElMessage({
                            type: 'success',
                            message: '删除成功',
    ...
    const excelData = () => {
        const params = {}
        Api.excelData4logaccountchangepassword(params).then(data => {
            const blob = new Blob([data.data], { type: 'application/vnd.ms-excel' })
            const blobUrl = URL.createObjectURL(blob)
            const a = document.createElement('a')
            a.href = blobUrl
    ...
    const toDelete = (scope) => {
        ElMessageBox.confirm(
            ...
            Api.del4logaccountchangepassword(scope.row.id).then(res => {
                console.log(res)
                if (res.code === 200){
                    ElMessage({
                        type: 'success',
